...
// void InsereF(hashF *hf, char *chave, int valor);
...
// 	for (int i = 0; i < M; i++) {
// 		hf->tabela[i].chave = (char *)malloc(M * sizeof(char));
// 		// hf->tabela[i].chave = -1;
// 		strcpy(hf->tabela[i].chave, "#");
...
// 	strcpy(hf->tabela[i].chave, chave);
// 	// hf->tabela[id].chave = chave;
// }

// void ImprimeF(hashF *hf) {
// 	for (int i = 0; i < hf->M; i++) {
// 		printf("%s -> ", hf->tabela[i].chave);
